...
import { SURG_MODS, SURG_PRE_Q, SURG_POST_Q, SURG_ROLES, SURG_RC } from '../data/surg_data.js';

function SurgeryCourse({ session, registered, onBack, onRegister, onGoHome }) {
  ...
  const PLANS = [
    { id: "full", label: "Full Course", ghc: 250, usd: 25, features: ["All 10 Modules", "Downloadable Notes", "Pre & Post Tests", "Student Discussion Board"] },
  ...
  const visibleMods = roleTab === "all" ? SURG_MODS : SURG_MODS.filter(m => m.aud.includes(roleTab));

  function submitPre() { let s = 0; SURG_PRE_Q.forEach((q, i) => { if (preAns[i] === q.ans) s++; }); setPreScore(s); setPreDone(true); }
  function submitPost() { let s = 0; SURG_POST_Q.forEach((q, i) => { if (postAns[i] === q.ans) s++; }); setPostScore(s); setPostDone(true); if (session) DB.push("completions", { ...session, postScore: s, preScore, course: "surgery" }); }
  ...
  if (readMod) return <ModuleReader mod={readMod} userRole={userRole} roleLabels={SURG_ROLES} roleColors={SURG_RC} onClose={() => setReadMod(null)} />;
    ...
    <div style={{ fontFamily: "'Georgia',serif", background: "#f8f6f0", minHeight: "100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        .su-nl{cursor:pointer;padding:7px 11px;border-radius:6px;transition:all .2s;font-family:'Source Sans 3',sans-serif;font-size:13px;font-weight:500;color:rgba(255,255,255,.82)}
        .su-nl:hover,.su-nl.act{color:#FFCDD2;background:rgba(255,205,210,.14)}
        .su-mob{display:none;background:none;border:none;cursor:pointer;color:#fff;font-size:24px}
        @media(max-width:820px){.su-dnav{display:none!important}.su-mob{display:flex!important}}
        button:disabled{opacity:.45;cursor:not-allowed}
      `}</style>

      {/* NAV */}
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, background: scrolled ? "#1a0505ee" : "#1a0505", backdropFilter:"blur(14px)", borderBottom:"2px solid #B71C1C1e", transition:"all .3s" }}>
      ...
      <div style={{ paddingTop:64 }}>
        ...
        {page==="home" && (
          <div>
            <div style={{ background:"linear-gradient(140deg,#1a0505,#7B0000,#B71C1C)", padding:"88px 24px 68px", textAlign:"center", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ maxWidth:900, margin:"0 auto", position:"relative" }}>
                ...
                <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,6vw,62px)", fontWeight:900, color:"#fff", lineHeight:1.08, margin:"16px 0 10px" }}>The Art of Cutting</h1>
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(14px,2.8vw,23px)", fontWeight:400, color:"#FFCDD2", marginBottom:18, fontStyle:"italic" }}>A Complete Course in Surgery</h2>
                <p style={{ color:"rgba(255,255,255,.82)", fontSize:"clamp(14px,2vw,17px)", maxWidth:700, margin:"0 auto 30px", lineHeight:1.9, fontFamily:"'Source Sans 3',sans-serif" }}>
                  From consent to closure, from the acute abdomen to the cancer operation — the complete surgery curriculum for medical, pharmacy, nursing and allied health students. Built around real West African clinical scenarios.
                </p>
            ...
            {/* Role selector */}
            <div style={{ background:"#fff5f5", padding:"52px 24px" }}>
              <div style={{ maxWidth:1100, margin:"0 auto", textAlign:"center" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:14 }}>
                  {Object.entries(SURG_ROLES)?.map(([k,l])=>(
                    <div key={k} onClick={()=>{ setPage("curriculum"); setRoleTab(k); }}
                      ...
                      <div style={{ fontWeight:700, fontSize:13, fontFamily:"'Source Sans 3',sans-serif", color:SURG_RC[k] }}>{l.slice(3)}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {page==="curriculum" && (
          <div style={{ maxWidth:1000, margin:"0 auto", padding:"48px 24px" }}>
            <div style={{ textAlign:"center", marginBottom:34 }}>
              ...
              <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(22px,4vw,36px)", fontWeight:700, color:"#1a1a2e", marginTop:14, marginBottom:10 }}>10 Modules · 70 Lessons · 25 Hours</h1>
            </div>
              ...
              <div key={m.id} style={{ background:"#fff", borderRadius:14, padding:"22px 20px", marginBottom:16, borderLeft:"6px solid "+m.color, boxShadow:"0 3px 16px rgba(183,28,28,.07)", transition:"all .25s", cursor:"pointer" }}
                ...
                {expMod===m.id && (
                  <div style={{ marginTop:18, paddingTop:18, borderTop:"1px solid #f0f0f0" }}>
                    ...
                    <button style={btn("primary",{ padding:"8px 18px", fontSize:13, background:"linear-gradient(135deg,"+m.color+",#7B0000)" })}
        ...
        {page==="register" && (
          <div style={{ maxWidth:660, margin:"0 auto", padding:"48px 24px" }}>
            ...
            {!registered ? (
              <div style={{ background:"#fff", borderRadius:18, padding:28, boxShadow:"0 4px 24px rgba(183,28,28,.08)", textAlign:"center" }}>
              ...
              <div style={{ background:"#fff", borderRadius:18, padding:28, boxShadow:"0 4px 24px rgba(183,28,28,.08)", textAlign:"center" }}>
                ...
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:22, color:"#B71C1C", marginBottom:8 }}>Welcome, {session?.name?.split(" ")[0]}!</h2>
        ...
        {page==="pretest" && (
          <div style={{ maxWidth:800, margin:"0 auto", padding:"48px 24px" }}>
            ...
            {preDone ? (
              <div style={{ background:"#fff", borderRadius:18, padding:28, textAlign:"center", boxShadow:"0 4px 24px rgba(183,28,28,.08)" }}>
                <div style={{ fontSize:48, marginBottom:12 }}>{preScore>=4?"🏆":preScore>=2?"📚":"🌱"}</div>
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:22, color:"#B71C1C", marginBottom:8 }}>Baseline: {preScore}/5</h2>
                <p style={{ color:"#5a6a8a", fontFamily:"'Source Sans 3',sans-serif", fontSize:14.5, lineHeight:1.85, marginBottom:18 }}>
                  {preScore>=4?"Strong foundation. This course will sharpen your surgical decision-making.":"Every concept is built from first principles — you are in exactly the right place."}
                </p>
                <button style={btn()} onClick={()=>setPage("curriculum")}>Begin the Course →</button>
              </div>
            ) : (
              <Quiz qs={SURG_PRE_Q} ans={preAns} setAns={setPreAns} done={preDone} onSubmit={submitPre} onSkip={()=>{ setPreDone(true); setPreScore(0); }} />
            )}
          </div>
        )}
          ...
          <div style={{ maxWidth:800, margin:"0 auto", padding:"48px 24px" }}>
            ...
            {postDone ? (
              <div style={{ background:"#fff", borderRadius:18, padding:28, textAlign:"center", boxShadow:"0 4px 24px rgba(183,28,28,.08)" }}>
                ...
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:22, color:postScore>=3?"#0a7c4a":"#B71C1C", marginBottom:8 }}>
              ...
              <Quiz qs={SURG_POST_Q} ans={postAns} setAns={setPostAns} done={postDone} onSubmit={submitPost} onSkip={null} />
            )}
          </div>
        )}
          ...
          <div style={{ maxWidth:820, margin:"0 auto", padding:"48px 24px" }}>
            ...
            {!postDone||postScore<3 ? (
              <div style={{ background:"#fff", borderRadius:18, padding:28, textAlign:"center", boxShadow:"0 4px 24px rgba(183,28,28,.08)" }}>
                ...
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:20, color:"#B71C1C", marginBottom:8 }}>Complete the Post-Course Assessment First</h2>
                <button style={btn()} onClick={()=>setPage("posttest")}>Take Assessment →</button>
              </div>
            ) : (
              <div>
                <div style={{ background:"#fff", borderRadius:18, padding:"42px 38px", boxShadow:"0 8px 40px rgba(183,28,28,.12)", border:"3px solid #FFCDD2", textAlign:"center" }}>
                  <div style={{ display:"flex", justifyContent:"center", marginBottom:12 }}><UGLogo size={64} /></div>
                  <div style={{ fontFamily:"'Source Sans 3',sans-serif", fontSize:10, letterSpacing:3, textTransform:"uppercase", color:"#B71C1C", marginBottom:5 }}>LegonMed Medical School</div>
                  ...
                  <div style={{ margin:"0 auto 9px", maxWidth:460 }}>
                    <input style={{ width:"100%", padding:"8px 0", textAlign:"center", fontFamily:"'Playfair Display',serif", fontSize:21, fontWeight:700, color:"#1a1a2e", border:"none", borderBottom:"2px solid #FFCDD2", borderRadius:0, background:"transparent", outline:"none" }}
      ...
      <footer style={{ background:"#1a0505", color:"rgba(255,255,255,.7)", padding:"24px", marginTop:40, textAlign:"center", fontFamily:"'Source Sans 3',sans-serif", fontSize:12 }}>
        <div style={{ fontFamily:"'Playfair Display',serif", fontSize:16, fontWeight:900, color:"#fff", marginBottom:4 }}>Legon<span style={{ color:"#FFCDD2" }}>Med</span> Medical School</div>
        The Art of Cutting · Surgery · © 2025 LegonMed · Evidence: Bailey & Love · ATLS · Hamilton Bailey's · WHO Surgical Safety Checklist
      </footer>
    </div>
  );
}

export default SurgeryCourse;
