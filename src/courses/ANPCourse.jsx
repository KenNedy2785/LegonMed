...
import {
  ANP_MODS,
  ANP_PRE_Q_M1, ANP_POST_Q_M1,
  ANP_PRE_Q_M2, ANP_POST_Q_M2,
  ANP_PRE_Q_M3, ANP_POST_Q_M3,
  ANP_PRE_Q_M4, ANP_POST_Q_M4,
  ANP_PRE_Q_M5, ANP_POST_Q_M5,
  ANP_PRE_Q_M6, ANP_POST_Q_M6,
} from '../data/anp_data.js';

const QUIZ_BANKS = {
  1:{pre:ANP_PRE_Q_M1, post:ANP_POST_Q_M1},
  2:{pre:ANP_PRE_Q_M2, post:ANP_POST_Q_M2},
  3:{pre:ANP_PRE_Q_M3, post:ANP_POST_Q_M3},
  4:{pre:ANP_PRE_Q_M4, post:ANP_POST_Q_M4},
  5:{pre:ANP_PRE_Q_M5, post:ANP_POST_Q_M5},
  6:{pre:ANP_PRE_Q_M6, post:ANP_POST_Q_M6},
};

const COURSE_COLOR = "#1B4F72";

export default function ANPCourse({ session, registered, onBack, onRegister, onGoHome }) {
  ...
  function submitQuiz(){
    ...
    setScore(s);
    setDone(true);
    if(session) DB.push("completions",{...session, course:"autonomic-pharmacology", module:activeMod.id, stage:quizStage, score:s, total:bank.length});
  }
    ...
    <div style={{fontFamily:"'Georgia',serif",background:C.off,minHeight:"100vh"}}>
      ...
      <div style={{background:`linear-gradient(140deg,${C.dark},${COURSE_COLOR},#0d2d45)`,padding:"60px 24px 50px",textAlign:"center"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          ...
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(28px,5.5vw,52px)",fontWeight:900,color:"#fff",lineHeight:1.1,margin:"16px 0 8px"}}>Autonomic Pharmacology</h1>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(13px,2.5vw,20px)",fontWeight:400,color:C.gold,marginBottom:14,fontStyle:"italic"}}>The Nervous System's Control Panel</h2>
          <p style={{color:"rgba(255,255,255,.82)",fontSize:"clamp(13px,2vw,16px)",maxWidth:640,margin:"0 auto",lineHeight:1.8,fontFamily:"'Source Sans 3',sans-serif"}}>
            Sympathetic and parasympathetic systems, receptors, agonists and antagonists — and how it all comes together in real patients. Module 1 is free.
          </p>
          <div style={{display:"flex",gap:20,justifyContent:"center",marginTop:30,flexWrap:"wrap"}}>
            {[["6","Modules"],["14h","Content"],["36","Lessons"],["Free","Module 1"]].map(([n,l])=>(
      ...
      <footer style={{background:C.dark,color:"rgba(255,255,255,.7)",padding:"24px",marginTop:30,textAlign:"center",fontFamily:"'Source Sans 3',sans-serif",fontSize:12}}>
        ...
        Autonomic Pharmacology · The Nervous System's Control Panel · © 2025 LegonMed
      </footer>
    </div>
  );
}
