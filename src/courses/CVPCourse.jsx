...
import {
  CVP_MODS,
  CVP_PRE_Q_M1, CVP_POST_Q_M1,
  CVP_PRE_Q_M2, CVP_POST_Q_M2,
  CVP_PRE_Q_M3, CVP_POST_Q_M3,
  CVP_PRE_Q_M4, CVP_POST_Q_M4,
  CVP_PRE_Q_M5, CVP_POST_Q_M5,
  CVP_PRE_Q_M6, CVP_POST_Q_M6,
  CVP_PRE_Q_M7, CVP_POST_Q_M7,
  CVP_PRE_Q_M8, CVP_POST_Q_M8,
  CVP_PRE_Q_M9, CVP_POST_Q_M9,
  CVP_PRE_Q_M10, CVP_POST_Q_M10,
} from '../data/cvp_data.js';

const QUIZ_BANKS = {
  1:{pre:CVP_PRE_Q_M1,post:CVP_POST_Q_M1},
  2:{pre:CVP_PRE_Q_M2,post:CVP_POST_Q_M2},
  3:{pre:CVP_PRE_Q_M3,post:CVP_POST_Q_M3},
  4:{pre:CVP_PRE_Q_M4,post:CVP_POST_Q_M4},
  5:{pre:CVP_PRE_Q_M5,post:CVP_POST_Q_M5},
  6:{pre:CVP_PRE_Q_M6,post:CVP_POST_Q_M6},
  7:{pre:CVP_PRE_Q_M7,post:CVP_POST_Q_M7},
  8:{pre:CVP_PRE_Q_M8,post:CVP_POST_Q_M8},
  9:{pre:CVP_PRE_Q_M9,post:CVP_POST_Q_M9},
  10:{pre:CVP_PRE_Q_M10,post:CVP_POST_Q_M10},
};

const COURSE_COLOR = "#922B21";

export default function CVPCourse({ session, registered, onBack, onRegister, onGoHome }) {
  ...
  function submitQuiz(){
    ...
    if(session) DB.push("completions",{...session, course:"cardiovascular-pharmacology", module:activeMod.id, stage:quizStage, score:s, total:bank.length});
  }
  ...
  return (
    <div style={{fontFamily:"'Georgia',serif",background:C.off,minHeight:"100vh"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <div style={{background:`linear-gradient(140deg,${C.dark},${COURSE_COLOR},#3d0000)`,padding:"60px 24px 50px",textAlign:"center"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          ...
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(28px,5.5vw,52px)",fontWeight:900,color:"#fff",lineHeight:1.1,margin:"16px 0 8px"}}>Cardiovascular Pharmacology</h1>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(13px,2.5vw,20px)",fontWeight:400,color:C.gold,marginBottom:14,fontStyle:"italic"}}>Drugs for the Heart</h2>
          <p style={{color:"rgba(255,255,255,.82)",fontSize:"clamp(13px,2vw,16px)",maxWidth:640,margin:"0 auto",lineHeight:1.8,fontFamily:"'Source Sans 3',sans-serif"}}>
            From antihypertensives to thrombolytics, anticoagulants to lipid-lowering agents — a complete guide to pharmacology of the cardiovascular system and blood. Module 1 is free.
          </p>
          <div style={{display:"flex",gap:20,justifyContent:"center",marginTop:30,flexWrap:"wrap"}}>
            {[["10","Modules"],["24h","Content"],["58","Lessons"],["Free","Module 1"]].map(([n,l])=>(
      ...
      <footer style={{background:C.dark,color:"rgba(255,255,255,.7)",padding:"24px",marginTop:30,textAlign:"center",fontFamily:"'Source Sans 3',sans-serif",fontSize:12}}>
        ...
        Cardiovascular Pharmacology · Drugs for the Heart · © 2025 LegonMed
      </footer>
    </div>
  );
}
