...
import {
  CHEMO_MODS,
  CHEMO_PRE_Q_M1, CHEMO_POST_Q_M1,
  CHEMO_PRE_Q_M2, CHEMO_POST_Q_M2,
  CHEMO_PRE_Q_M3, CHEMO_POST_Q_M3,
  CHEMO_PRE_Q_M4, CHEMO_POST_Q_M4,
  CHEMO_PRE_Q_M5, CHEMO_POST_Q_M5,
  CHEMO_PRE_Q_M6, CHEMO_POST_Q_M6,
  CHEMO_PRE_Q_M7, CHEMO_POST_Q_M7,
  CHEMO_PRE_Q_M8, CHEMO_POST_Q_M8,
  CHEMO_PRE_Q_M9, CHEMO_POST_Q_M9,
} from '../data/chemo_data.js';

const QUIZ_BANKS = {
  1:{pre:CHEMO_PRE_Q_M1,post:CHEMO_POST_Q_M1},
  2:{pre:CHEMO_PRE_Q_M2,post:CHEMO_POST_Q_M2},
  3:{pre:CHEMO_PRE_Q_M3,post:CHEMO_POST_Q_M3},
  4:{pre:CHEMO_PRE_Q_M4,post:CHEMO_POST_Q_M4},
  5:{pre:CHEMO_PRE_Q_M5,post:CHEMO_POST_Q_M5},
  6:{pre:CHEMO_PRE_Q_M6,post:CHEMO_POST_Q_M6},
  7:{pre:CHEMO_PRE_Q_M7,post:CHEMO_POST_Q_M7},
  8:{pre:CHEMO_PRE_Q_M8,post:CHEMO_POST_Q_M8},
  9:{pre:CHEMO_PRE_Q_M9,post:CHEMO_POST_Q_M9},
};

const COURSE_COLOR = "#1B4F72";

export default function CHEMOCourse({ session, registered, onBack, onRegister, onGoHome }) {
  ...
  function openModule(mod){ setActiveMod(mod); setView("reader"); }
  function openQuiz(mod, stage){
    setActiveMod(mod); setQuizStage(stage);
    setAns({}); setDone(false); setScore(null); setView("quiz");
  }
  function submitQuiz(){
    ...
    if(session) DB.push("completions",{...session, course:"chemotherapeutic-agents", module:activeMod.id, stage:quizStage, score:s, total:bank.length});
  }

  if(view==="reader" && activeMod) return <ModuleReader mod={activeMod} userRole={userRole} onClose={()=>setView("home")}/>;
      ...
      <div style={{fontFamily:"'Georgia',serif",background:C.off,minHeight:"100vh"}}>
        ...
        <div style={{background:`linear-gradient(135deg,${C.dark},${activeMod.color})`,padding:"18px 24px"}}>
          <div style={{maxWidth:800,margin:"0 auto"}}>
            <div style={{color:C.gold,fontFamily:"'Source Sans 3',sans-serif",fontSize:11,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>Module {activeMod.num} · {quizStage==="pre"?"Pre-Test":"Post-Test"}</div>
        ...
        <div style={{maxWidth:800,margin:"0 auto",padding:"38px 24px 80px"}}>
          {done ? (
            <div style={{background:"#fff",borderRadius:18,padding:28,textAlign:"center",boxShadow:"0 4px 24px rgba(0,48,135,.08)",marginBottom:20}}>
              ...
              <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:22,color:C.blue,marginBottom:8}}>{quizStage==="pre"?"Baseline":"Score"}: {score}/{bank.length}</h2>
  ...
  return (
    <div style={{fontFamily:"'Georgia',serif",background:C.off,minHeight:"100vh"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <div style={{background:`linear-gradient(140deg,${C.dark},${COURSE_COLOR},#0a1628)`,padding:"60px 24px 50px",textAlign:"center"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"center",marginBottom:16}}><UGLogo size={70}/></div>
          <span style={bdg}>⚔️ Core Pharmacology · LegonMed Pharmacology Institute</span>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(28px,5.5vw,52px)",fontWeight:900,color:"#fff",lineHeight:1.1,margin:"16px 0 8px"}}>Chemotherapeutic Agents</h1>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(13px,2.5vw,20px)",fontWeight:400,color:C.gold,marginBottom:14,fontStyle:"italic"}}>Fighting Infection & Cancer</h2>
          <p style={{color:"rgba(255,255,255,.82)",fontSize:"clamp(13px,2vw,16px)",maxWidth:640,margin:"0 auto",lineHeight:1.8,fontFamily:"'Source Sans 3',sans-serif"}}>
            From antibiotics to antivirals, antimalarials to anticancer biologics — a comprehensive guide to drugs that target pathogens and tumours while sparing the host. Module 1 is free.
          </p>
          <div style={{display:"flex",gap:20,justifyContent:"center",marginTop:30,flexWrap:"wrap"}}>
            {[["9","Modules"],["20h","Content"],["54","Lessons"],["Free","Module 1"]].map(([n,l])=>(
      ...
      <footer style={{background:C.dark,color:"rgba(255,255,255,.7)",padding:"24px",marginTop:30,textAlign:"center",fontFamily:"'Source Sans 3',sans-serif",fontSize:12}}>
        ...
        Chemotherapeutic Agents · Fighting Infection & Cancer · © 2025 LegonMed
      </footer>
    </div>
  );
}
