...
import { IIP_MODS, IIP_PRE_Q, IIP_POST_Q, IIP_ROLES, IIP_RC } from '../data/iip_data.js';

function IIPCourse({ session, registered, onBack, onRegister, onGoHome }) {
  ...
  const PLANS = [
    { id:"full", label:"Full Course", ghc:280, usd:28, features:["All 17 Modules","Downloadable Notes","Pre & Post Tests","Student Discussion Board"] },
    { id:"cert", label:"Course + Certificate", ghc:380, usd:38, featured:true, features:["Everything in Full","Official LegonMed Certificate","3 Signatories","Digital Badge","Verifiable QR Code"] },
    { id:"inst", label:"Institutional (30 seats)", ghc:3800, usd:380, features:["30 Licences","Admin Dashboard","Group Certificates","Priority Support"] },
  ];

  const visibleMods = roleTab === "all" ? IIP_MODS : IIP_MODS.filter(m => m.aud.includes(roleTab));

  function submitPre() { let s = 0; IIP_PRE_Q.forEach((q,i) => { if (preAns[i] === q.ans) s++; }); setPreScore(s); setPreDone(true); }
  function submitPost() { let s = 0; IIP_POST_Q.forEach((q,i) => { if (postAns[i] === q.ans) s++; }); setPostScore(s); setPostDone(true); if (session) DB.push("completions", { ...session, postScore:s, preScore, course:"iip" }); }
  ...
  if (readMod) return <ModuleReader mod={readMod} userRole={userRole} roleLabels={IIP_ROLES} roleColors={IIP_RC} onClose={() => setReadMod(null)} />;
    ...
    <div style={{ fontFamily:"'Georgia',serif", background:"#f8f6f0", minHeight:"100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        .ii-nl{cursor:pointer;padding:7px 11px;border-radius:6px;transition:all .2s;font-family:'Source Sans 3',sans-serif;font-size:13px;font-weight:500;color:rgba(255,255,255,.82)}
        .ii-nl:hover,.ii-nl.act{color:#C5CAE9;background:rgba(197,202,233,.14)}
        .ii-mob{display:none;background:none;border:none;cursor:pointer;color:#fff;font-size:24px}
        @media(max-width:820px){.ii-dnav{display:none!important}.ii-mob{display:flex!important}}
        button:disabled{opacity:.45;cursor:not-allowed}
      `}</style>

      {/* NAV */}
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, background:scrolled?"#0d0d2bee":"#0d0d2b", backdropFilter:"blur(14px)", borderBottom:"2px solid #1A237E1e", transition:"all .3s" }}>
        ...
        {mob && <div style={{ background:"#0d0d2b", padding:"14px 20px", borderTop:"1px solid #1A237E28" }}>
          {NAV.map(n => <div key={n.id} className="ii-nl" style={{ display:"block", marginBottom:8 }} onClick={() => { setPage(n.id); setMob(false); }}>{n.l}</div>)}
        </div>}
      </nav>
          ...
          <div>
            <div style={{ background:"linear-gradient(140deg,#0d0d2b,#1A237E,#283593)", padding:"88px 24px 68px", textAlign:"center", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ maxWidth:900, margin:"0 auto", position:"relative" }}>
                ...
                <p style={{ color:"rgba(255,255,255,.82)", fontSize:"clamp(14px,2vw,17px)", maxWidth:720, margin:"0 auto 30px", lineHeight:1.9, fontFamily:"'Source Sans 3',sans-serif" }}>
                  From aspirin to monoclonal antibodies, from corticosteroids to CAR-T cells — the complete pharmacology of immune modulation. The drugs that tame inflammation, suppress autoimmunity, prevent transplant rejection, and harness the immune system against cancer.
                </p>
                <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
                  <button style={btn("primary",{ fontSize:16, padding:"14px 32px" })} onClick={() => setPage("curriculum")}>🛡️ View Curriculum</button>
                ...
                <div style={{ display:"flex", gap:24, justifyContent:"center", marginTop:44, flexWrap:"wrap" }}>
                  {[["17","Modules"],["38h","Content"],["100+","Lessons"],["5","Professions"],["Free","Module 1"]].map(([n,l]) => (
            ...
            <div style={{ background:"#fff", padding:"52px 24px" }}>
              <div style={{ maxWidth:1200, margin:"0 auto", textAlign:"center" }}>
                ...
                <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(22px,4vw,38px)", fontWeight:700, color:"#1a1a2e", marginTop:14, marginBottom:32 }}>17 Modules. From Aspirin to Immunotherapy.</h2>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
                  {IIP_MODS.map(m => (
                    <div key={m.id} style={{ background:"#f0f0ff", borderRadius:14, padding:"18px 14px", borderLeft:"4px solid "+m.color, textAlign:"left", cursor:"pointer", transition:"all .25s", boxShadow:"0 2px 10px rgba(26,35,126,.06)" }}
                      onMouseOver={e => e.currentTarget.style.transform="translateY(-3px)"}
                      onMouseOut={e => e.currentTarget.style.transform=""}
                      onClick={() => setPage("curriculum")}>
                      <div style={{ fontSize:22, marginBottom:6 }}>{m.icon}</div>
                      <div style={{ fontFamily:"'Playfair Display',serif", fontSize:13, fontWeight:700, color:m.color, marginBottom:3 }}>Module {m.num}</div>
                      <div style={{ fontFamily:"'Source Sans 3',sans-serif", fontSize:12, color:"#1a1a2e", fontWeight:600, marginBottom:2 }}>{m.title}</div>
                      <div style={{ fontFamily:"'Source Sans 3',sans-serif", fontSize:11, color:"#5a6a8a" }}>{m.sub}</div>
                      {m.free && <span style={{ background:"#0a7c4a", color:"#fff", padding:"2px 8px", borderRadius:20, fontSize:10, fontWeight:700, fontFamily:"'Source Sans 3',sans-serif", display:"inline-block", marginTop:6 }}>FREE</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ background:"#f0f0ff", padding:"52px 24px" }}>
              <div style={{ maxWidth:1100, margin:"0 auto", textAlign:"center" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:14 }}>
                  {Object.entries(IIP_ROLES).map(([k,l]) => (
                    <div key={k} onClick={() => { setPage("curriculum"); setRoleTab(k); }}
                      style={{ padding:"20px 14px", borderRadius:14, border:"3px solid "+IIP_RC[k], background:"#fff", cursor:"pointer", textAlign:"center", transition:"all .25s" }}
                      onMouseOver={e => { e.currentTarget.style.background=IIP_RC[k]; e.currentTarget.style.color="#fff"; }}
                      ...
                      <div style={{ fontWeight:700, fontSize:13, fontFamily:"'Source Sans 3',sans-serif", color:IIP_RC[k] }}>{l.slice(3)}</div>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {page==="curriculum" && (
          <div style={{ maxWidth:1000, margin:"0 auto", padding:"48px 24px" }}>
            <div style={{ textAlign:"center", marginBottom:34 }}>
              ...
              <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(22px,4vw,36px)", fontWeight:700, color:"#1a1a2e", marginTop:14, marginBottom:10 }}>17 Modules · 100+ Lessons · 38 Hours</h1>
            </div>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:26 }}>
              ...
              {Object.entries(IIP_ROLES).map(([k,l]) => (
                <button key={k} onClick={() => setRoleTab(k)} style={{ padding:"7px 14px", borderRadius:30, border:"2px solid "+IIP_RC[k], background:roleTab===k?IIP_RC[k]:"#fff", color:roleTab===k?"#fff":IIP_RC[k], fontFamily:"'Source Sans 3',sans-serif", fontSize:13, fontWeight:600, cursor:"pointer", transition:"all .2s" }}>{l.split(" ").slice(1,3).join(" ")}</button>
              ))}
            </div>
            {visibleMods.map(m => (
              <div key={m.id} style={{ background:"#fff", borderRadius:14, padding:"22px 20px", marginBottom:16, borderLeft:"6px solid "+m.color, boxShadow:"0 3px 16px rgba(26,35,126,.07)", transition:"all .25s", cursor:"pointer" }}
                ...
                {expMod===m.id && (
                  <div style={{ marginTop:18, paddingTop:18, borderTop:"1px solid #f0f0f0" }}>
                    ...
                    <button style={btn("primary",{ padding:"8px 18px", fontSize:13, background:"linear-gradient(135deg,"+m.color+",#283593)" })}
                      ...
                      {m.free||registered||isAdmin()?"🛡️ Read Module":"🎓 Enroll to Access"}
                    </button>
                  </div>
                )}
              </div>
            ...
            {preDone ? (
              <div style={{ background:"#fff", borderRadius:18, padding:28, textAlign:"center", boxShadow:"0 4px 24px rgba(26,35,126,.08)" }}>
                ...
                <p style={{ color:"#5a6a8a", fontFamily:"'Source Sans 3',sans-serif", fontSize:14.5, lineHeight:1.85, marginBottom:18 }}>
                  {preScore>=4?"Strong foundation. This course will sharpen your immunopharmacology precision.":"Every concept is built from first principles — you are in exactly the right place."}
                </p>
                <button style={btn()} onClick={() => setPage("curriculum")}>Begin the Course →</button>
              </div>
            ) : (
              <Quiz qs={IIP_PRE_Q} ans={preAns} setAns={setPreAns} done={preDone} onSubmit={submitPre} onSkip={() => { setPreDone(true); setPreScore(0); }} />
            )}
          </div>
        )}
        ...
        {page==="posttest" && (
          <div style={{ maxWidth:800, margin:"0 auto", padding:"48px 24px" }}>
            ...
            {postDone ? (
              ...
              <Quiz qs={IIP_POST_Q} ans={postAns} setAns={setPostAns} done={postDone} onSubmit={submitPost} onSkip={null} />
            )}
          </div>
        )}
              ...
              <div>
                <div style={{ background:"#fff", borderRadius:18, padding:"42px 38px", boxShadow:"0 8px 40px rgba(26,35,126,.12)", border:"3px solid #C5CAE9", textAlign:"center" }}>
                  ...
                  <div style={{ margin:"0 auto 9px", maxWidth:460 }}>
                    <input style={{ width:"100%", padding:"8px 0", textAlign:"center", fontFamily:"'Playfair Display',serif", fontSize:21, fontWeight:700, color:"#1a1a2e", border:"none", borderBottom:"2px solid #C5CAE9", borderRadius:0, background:"transparent", outline:"none" }}
                  ...
                  <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(17px,3vw,26px)", fontWeight:700, color:"#1a1a2e", marginBottom:4 }}>Taming the Immune Response</h2>
                  <div style={{ fontFamily:"'Source Sans 3',sans-serif", fontSize:13, color:"#1A237E", marginBottom:18 }}>Inflammo- & Immunopharmacology · LegonMed Medical School</div>
      ...
      <footer style={{ background:"#0d0d2b", color:"rgba(255,255,255,.7)", padding:"24px", marginTop:40, textAlign:"center", fontFamily:"'Source Sans 3',sans-serif", fontSize:12 }}>
        ...
        Taming the Immune Response · Inflammo- & Immunopharmacology · © 2025 LegonMed · Evidence: Rang & Dale · Goodman & Gilman · EULAR Guidelines
      </footer>
    </div>
  );
}

export default IIPCourse;
