...
import { C, btn, bdg, UGLogo, DB, isAdmin } from '../shared.jsx';
import { Quiz, ModuleReader } from '../SharedUI.jsx';
import {
  DD_MODS,
  DD_PRE_Q_M1, DD_POST_Q_M1,
  DD_PRE_Q_M2, DD_POST_Q_M2,
  DD_PRE_Q_M3, DD_POST_Q_M3,
  DD_PRE_Q_M4, DD_POST_Q_M4,
  DD_PRE_Q_M5, DD_POST_Q_M5,
} from '../data/dd_data.js';

const QUIZ_BANKS = {
  1:{pre:DD_PRE_Q_M1,post:DD_POST_Q_M1},
  2:{pre:DD_PRE_Q_M2,post:DD_POST_Q_M2},
  3:{pre:DD_PRE_Q_M3,post:DD_POST_Q_M3},
  4:{pre:DD_PRE_Q_M4,post:DD_POST_Q_M4},
  5:{pre:DD_PRE_Q_M5,post:DD_POST_Q_M5},
};

const COURSE_COLOR = "#1B4F72";

export default function DDCourse({ session, registered, onBack, onRegister }) {
  ...
  function submitQuiz(){
    ...
    if(session) DB.push("completions",{...session, course:"drug-discovery", module:activeMod.id, stage:quizStage, score:s, total:bank.length});
  }
  ...
  return (
    <div style={{fontFamily:"'Georgia',serif",background:C.off,minHeight:"100vh"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <div style={{background:`linear-gradient(140deg,${C.dark},${COURSE_COLOR},#000814)`,padding:"60px 24px 50px",textAlign:"center"}}>
        <div style={{maxWidth:900,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"center",marginBottom:16}}><UGLogo size={70}/></div>
          <span style={bdg}>🔬 Advanced Pharmacology · LegonMed Pharmacology Institute</span>
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(28px,5.5vw,52px)",fontWeight:900,color:"#fff",lineHeight:1.1,margin:"16px 0 8px"}}>Drug Discovery</h1>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:"clamp(13px,2.5vw,20px)",fontWeight:400,color:C.gold,marginBottom:14,fontStyle:"italic"}}>From Lab to Lead</h2>
          <p style={{color:"rgba(255,255,255,.82)",fontSize:"clamp(13px,2vw,16px)",maxWidth:640,margin:"0 auto",lineHeight:1.8,fontFamily:"'Source Sans 3',sans-serif"}}>
            From target identification to regulatory approval — the complete drug discovery pipeline, with a focus on neglected tropical diseases, African medicinal plants, and the science behind every drug you will ever prescribe. Module 1 is free.
          </p>
          <div style={{display:"flex",gap:20,justifyContent:"center",marginTop:30,flexWrap:"wrap"}}>
            {[["5","Modules"],["12h","Content"],["30","Lessons"],["Free","Module 1"]].map(([n,l])=>(
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"40px 24px"}}>
        ...
        <div style={{textAlign:"center",marginTop:20}}>
          <button style={btn("secondary")} onClick={onBack}>← Back to Advanced Pharmacology</button>
        </div>
      </div>
      <footer style={{background:C.dark,color:"rgba(255,255,255,.7)",padding:"24px",marginTop:30,textAlign:"center",fontFamily:"'Source Sans 3',sans-serif",fontSize:12}}>
        ...
        Drug Discovery · From Lab to Lead · © 2025 LegonMed
      </footer>
    </div>
  );
}
