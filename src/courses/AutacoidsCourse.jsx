...
import {
  AUT_MODS,
  AUT_PRE_Q_M1, AUT_POST_Q_M1,
  AUT_PRE_Q_M2, AUT_POST_Q_M2,
  AUT_PRE_Q_M3, AUT_POST_Q_M3,
  AUT_PRE_Q_M4, AUT_POST_Q_M4,
  AUT_PRE_Q_M5, AUT_POST_Q_M5,
  AUT_PRE_Q_M6, AUT_POST_Q_M6,
  AUT_PRE_Q_M7, AUT_POST_Q_M7,
} from '../data/aut_data.js';

const COURSE_COLOR = "#1a237e";
const COURSE_ACCENT = "#e65100";
...
const PRE_QS = [
  AUT_PRE_Q_M1, AUT_PRE_Q_M2, AUT_PRE_Q_M3, AUT_PRE_Q_M4,
  AUT_PRE_Q_M5, AUT_PRE_Q_M6, AUT_PRE_Q_M7,
];
const POST_QS = [
  AUT_POST_Q_M1, AUT_POST_Q_M2, AUT_POST_Q_M3, AUT_POST_Q_M4,
  AUT_POST_Q_M5, AUT_POST_Q_M6, AUT_POST_Q_M7,
];

// Flatten all pre/post questions into one array for the overall assessments
const ALL_PRE_Q = PRE_QS.flat();
const ALL_POST_Q = POST_QS.flat();

function AutacoidsCourse({ session, registered, onBack, onRegister, onGoHome }) {
  ...
  const PLANS = [
    { id: "full", label: "Full Course", ghc: 250, usd: 25, features: ["All 7 Modules", "Downloadable Notes", "Pre & Post Tests", "Student Discussion Board"] },
  ...
  const visibleMods = roleTab === "all" ? AUT_MODS : AUT_MODS.filter(m => m.aud.includes(roleTab));

  // Use a sample of questions for the overall pre/post test (first Q from each module = 7 questions)
  ...
  function submitPost() {
    ...
    if (session) DB.push("completions", { ...session, postScore: s, preScore, course: "autacoids" });
  }
  ...
  if (readMod) return <ModuleReader mod={readMod} userRole={userRole} roleLabels={AUT_ROLES} roleColors={AUT_RC} onClose={() => setReadMod(null)} />;
    ...
    <div style={{ fontFamily: "'Georgia',serif", background: "#f8f6f0", minHeight: "100vh" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        .au-nl{cursor:pointer;padding:7px 11px;border-radius:6px;transition:all .2s;font-family:'Source Sans 3',sans-serif;font-size:13px;font-weight:500;color:rgba(255,255,255,.82)}
        .au-nl:hover,.au-nl.act{color:#e65100;background:rgba(230,81,0,.14)}
        .au-mob{display:none;background:none;border:none;cursor:pointer;color:#fff;font-size:24px}
        @media(max-width:820px){.au-dnav{display:none!important}.au-mob{display:flex!important}}
        button:disabled{opacity:.45;cursor:not-allowed}
      `}</style>

      {/* NAV */}
      <nav style={{ position:"fixed",top:0,left:0,right:0,zIndex:1000,background:scrolled?"#0d1b5eee":"#0d1b5e",backdropFilter:"blur(14px)",borderBottom:"2px solid #e6510018",transition:"all .3s" }}>
        ...
        {mob && <div style={{ background:"#0d1b5e",padding:"14px 20px",borderTop:"1px solid #e6510028" }}>
          {NAV.map(n=><div key={n.id} className="au-nl" style={{ display:"block",marginBottom:8 }} onClick={()=>{setPage(n.id);setMob(false);}}>{n.l}</div>)}
        </div>}
      </nav>
      ...
      <div style={{ paddingTop:64 }}>
        ...
        {page==="home" && (
          <div>
            {/* HERO */}
            <div style={{ background:"linear-gradient(140deg,#0d1b5e,#1a237e,#311b92)",padding:"88px 24px 68px",textAlign:"center",position:"relative",overflow:"hidden" }}>
              ...
              <div style={{ maxWidth:900,margin:"0 auto",position:"relative" }}>
                ...
                <p style={{ color:"rgba(255,255,255,.82)",fontSize:"clamp(14px,2vw,17px)",maxWidth:680,margin:"0 auto 30px",lineHeight:1.9,fontFamily:"'Source Sans 3',sans-serif" }}>
                  The complete autacoids course — from the itch of a mosquito bite to pulmonary hypertension, from aspirin's mechanisms to the pharmacogenomics of clopidogrel. Seven modules. Five professions. One integrated story of local chemical signalling.
                </p>
                <div style={{ display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap" }}>
                  <button style={btn("primary",{fontSize:16,padding:"14px 32px",background:"linear-gradient(135deg,#e65100,#bf360c)"})} onClick={()=>setPage("curriculum")}>📚 View Curriculum</button>
                ...
                <div style={{ display:"flex",gap:24,justifyContent:"center",marginTop:44,flexWrap:"wrap" }}>
                  {[["7","Modules"],["17h","Content"],["46","Lessons"],["5","Professions"],["Free","Module 1"]].map(([n,l])=>(
                    <div key={l} style={{ textAlign:"center" }}>
                      <div style={{ fontFamily:"'Playfair Display',serif",fontSize:32,fontWeight:900,color:"#e65100" }}>{n}</div>
            ...
            <div style={{ background:"#fff",padding:"52px 24px" }}>
              <div style={{ maxWidth:1100,margin:"0 auto",textAlign:"center" }}>
                ...
                <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:"clamp(20px,4vw,36px)",fontWeight:700,color:"#1a1a2e",marginTop:14,marginBottom:32 }}>7 Autacoid Families. From Allergy to Vasculature.</h2>
            ...
            <div style={{ background:"#f0f4ff",padding:"52px 24px" }}>
              <div style={{ maxWidth:1100,margin:"0 auto",textAlign:"center" }}>
                ...
                <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:"clamp(20px,4vw,36px)",fontWeight:700,color:"#1a1a2e",marginTop:14,marginBottom:32 }}>Tailored for All Health Professions</h2>
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:14 }}>
                  {Object.entries(AUT_ROLES).map(([k,l])=>(
                    <div key={k} onClick={()=>{setPage("curriculum");setRoleTab(k);}}
                      style={{ padding:"20px 14px",borderRadius:14,border:"3px solid "+AUT_RC[k],background:"#fff",cursor:"pointer",textAlign:"center",transition:"all .25s" }}
                      onMouseOver={e=>{e.currentTarget.style.background=AUT_RC[k];e.currentTarget.style.color="#fff";}}
                      ...
                      <div style={{ fontWeight:700,fontSize:13,fontFamily:"'Source Sans 3',sans-serif",color:AUT_RC[k] }}>{l.slice(3)}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ background:"#fff",padding:"52px 24px" }}>
              <div style={{ maxWidth:860,margin:"0 auto",textAlign:"center" }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16 }}>
                  {PLANS.map(p=>(
                    <div key={p.id} style={{ background:"#f8f6f0",borderRadius:18,padding:"28px 20px",boxShadow:"0 4px 20px rgba(13,27,94,.08)",textAlign:"center",position:"relative",border:p.featured?"2px solid #e65100":"1px solid #e8edf5" }}>
                      {p.featured && <div style={{ position:"absolute",top:-12,left:"50%",transform:"translateX(-50%)",background:"#e65100",color:"#fff",padding:"3px 14px",borderRadius:20,fontSize:11,fontWeight:700,fontFamily:"'Source Sans 3',sans-serif" }}>⭐ MOST POPULAR</div>}
                      <div style={{ fontFamily:"'Playfair Display',serif",fontSize:17,fontWeight:700,color:"#1a237e",marginBottom:6 }}>{p.label}</div>
                      <div style={{ fontSize:24,fontWeight:900,color:p.featured?"#e65100":"#1a1a2e",fontFamily:"'Playfair Display',serif",marginBottom:2 }}>GH₵ {p.ghc}</div>
                      ...
                      <button style={btn(p.featured?"primary":"secondary",{width:"100%",background:p.featured?"linear-gradient(135deg,#e65100,#bf360c)":undefined})} onClick={()=>setPage("register")}>Enroll Now</button>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {page==="curriculum" && (
          <div style={{ maxWidth:1000,margin:"0 auto",padding:"48px 24px" }}>
            <div style={{ textAlign:"center",marginBottom:34 }}>
              ...
              <h1 style={{ fontFamily:"'Playfair Display',serif",fontSize:"clamp(20px,4vw,34px)",fontWeight:700,color:"#1a1a2e",marginTop:14,marginBottom:10 }}>7 Modules · 46 Lessons · 17 Hours</h1>
            </div>
            <div style={{ display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",marginBottom:26 }}>
              ...
              {Object.entries(AUT_ROLES).map(([k,l])=>(
                <button key={k} onClick={()=>setRoleTab(k)} style={{ padding:"7px 14px",borderRadius:30,border:"2px solid "+AUT_RC[k],background:roleTab===k?AUT_RC[k]:"#fff",color:roleTab===k?"#fff":AUT_RC[k],fontFamily:"'Source Sans 3',sans-serif",fontSize:13,fontWeight:600,cursor:"pointer",transition:"all .2s" }}>{l.split(" ").slice(1,3).join(" ")}</button>
              ))}
            </div>
            {visibleMods.map(m=>(
              <div key={m.id} style={{ background:"#fff",borderRadius:14,padding:"22px 20px",marginBottom:16,borderLeft:"6px solid "+m.color,boxShadow:"0 3px 16px rgba(13,27,94,.07)",transition:"all .25s",cursor:"pointer" }}
                ...
                {expMod===m.id && (
                  <div style={{ marginTop:18,paddingTop:18,borderTop:"1px solid #f0f0f0" }}>
                    ...
                    <button style={btn("primary",{padding:"8px 18px",fontSize:13,background:"linear-gradient(135deg,"+m.color+",#283593)"})}
          ...
          <div style={{ maxWidth:660,margin:"0 auto",padding:"48px 24px" }}>
            ...
            {!registered ? (
              <div style={{ background:"#fff",borderRadius:18,padding:28,boxShadow:"0 4px 24px rgba(13,27,94,.08)",textAlign:"center" }}>
                ...
                <button style={btn("primary",{width:"100%",padding:14,background:"linear-gradient(135deg,#e65100,#bf360c)"})} onClick={onRegister}>🎓 Register Now →</button>
              </div>
            ) : (
              <div style={{ background:"#fff",borderRadius:18,padding:28,boxShadow:"0 4px 24px rgba(13,27,94,.08)",textAlign:"center" }}>
        ...
        {page==="pretest" && (
          <div style={{ maxWidth:800,margin:"0 auto",padding:"48px 24px" }}>
            <div style={{ textAlign:"center",marginBottom:28 }}>
              ...
              <p style={{ color:"#5a6a8a",fontFamily:"'Source Sans 3',sans-serif",fontSize:14 }}>7 questions · One per module · Baseline only · No grade</p>
            </div>
            {preDone ? (
              <div style={{ background:"#fff",borderRadius:18,padding:28,textAlign:"center",boxShadow:"0 4px 24px rgba(13,27,94,.08)" }}>
                <div style={{ fontSize:48,marginBottom:12 }}>{preScore>=6?"🏆":preScore>=4?"📚":"🌱"}</div>
                <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:22,color:"#1a237e",marginBottom:8 }}>Baseline: {preScore}/7</h2>
                <p style={{ color:"#5a6a8a",fontFamily:"'Source Sans 3',sans-serif",fontSize:14.5,lineHeight:1.85,marginBottom:18 }}>
                  {preScore>=6?"Excellent foundation. This course will deepen your clinical application.":"Great time to start. Every concept is built from first principles."}
                </p>
        ...
        {page==="posttest" && (
          <div style={{ maxWidth:800,margin:"0 auto",padding:"48px 24px" }}>
            <div style={{ textAlign:"center",marginBottom:28 }}>
              ...
              <p style={{ color:"#5a6a8a",fontFamily:"'Source Sans 3',sans-serif",fontSize:14 }}>7 clinical scenarios · One per module · Pass mark 60% · Required for certificate</p>
            </div>
            {postDone ? (
              <div style={{ background:"#fff",borderRadius:18,padding:28,textAlign:"center",boxShadow:"0 4px 24px rgba(13,27,94,.08)" }}>
                <div style={{ fontSize:48,marginBottom:12 }}>{postScore>=6?"🏆":postScore>=5?"✅":"📚"}</div>
                <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:22,color:postScore>=5?"#0a7c4a":"#1a237e",marginBottom:8 }}>
                  Score: {postScore}/7 — {postScore>=5?"PASS ✓":"Review Required"}
                </h2>
                {preScore!=null && <p style={{ color:"#5a6a8a",fontFamily:"'Source Sans 3',sans-serif",fontSize:14,marginBottom:12 }}>
                  Pre: {preScore}/7 → Post: {postScore}/7 {postScore>preScore?`(+${postScore-preScore} 🎯)`:""}
                </p>}
                {postScore>=5 && <button style={btn()} onClick={()=>setPage("certificate")}>🎓 Get Certificate →</button>}
              </div>
            ) : (
...
export default AutacoidsCourse;
