  ...
  {id:"med",num:"03",icon:"\uD83C\uDF93",name:"Medical School",tagline:"Preclinical to clinical — the complete medical curriculum.",color:"#003087",type:"education",
   ...
   categories:[
    ...
    {name:"Clinical Medicine",icon:"\uD83E\uDE7A",courses:[
      ...
      {id:"paed",title:"Paediatrics",sub:"Medicine for Children",status:"live",icon:"\uD83D\uDC76",dur:"20h",modules:8},
...
export default function LegonMed(){
  ...
  if(view==="course" && activeCourse==="dd") return(
    <>
      <style>{`@import url("https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap");*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <DDCourse session={session} registered={registered} onBack={()=>go("pillar",PILLARS[2])} onRegister={()=>setShowReg(true)} onGoHome={()=>go("home")}/>
      {showReg&&<RegModal/>}
    </>
  );
  ...
  if(view==="course"&&activeCourse==="ep") return(
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <EPCourse session={session} registered={registered} onBack={()=>go("pillar",PILLARS[0])} onRegister={()=>setShowReg(true)} onGoHome={()=>go("home")}/>
      {showReg&&<RegModal/>}
    </>
  );

  if(view==="course"&&activeCourse==="iip") return(
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <IIPCourse session={session} registered={registered} onBack={()=>go("pillar",PILLARS[2])} onRegister={()=>setShowReg(true)} onGoHome={()=>go("home")}/>
      {showReg&&<RegModal/>}
    </>
  );
  if(view==="course"&&activeCourse==="rp") return(
    <>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400&family=Source+Sans+3:wght@300;400;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0}button:disabled{opacity:.45;cursor:not-allowed}`}</style>
      <RespiratoryPharmCourse session={session} registered={registered} onBack={()=>go("pillar",PILLARS[1])} onRegister={()=>setShowReg(true)} onGoHome={()=>go("home")}/>
      {showReg&&<RegModal/>}
    </>
  );
